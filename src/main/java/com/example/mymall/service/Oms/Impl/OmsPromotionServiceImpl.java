...
import com.example.mymall.service.Oms.OmsPromotionService;
...
public class OmsPromotionServiceImpl implements OmsPromotionService {
			...
			if (promotionType == 1) {
				...
				if (fullReduction != null) {
					for (OmsCartItem item : itemList) {
						...
						cartPromotionItem.setPromotionMessage(message);
						//(商品原价/总价)*满减金额
						PmsSkuStock skuStock = getOriginalPrice(promotionProduct, cartPromotionItem.getProductSkuId());
						BigDecimal originalPrice = skuStock.getPrice();
	 ...
	 * 获取购物车中指定商品的总价
	 */
	private BigDecimal getCartItemAmount(List<OmsCartItem> itemList,
